...
function play(connection, message) {
  ...
  search(args.join(" "), opts, function(err, results) {
    ...
    message.channel.send(embed).then(embeds => {
        embeds.react(":one:").then(async r => {
            await embeds.react(":two:");
            await embeds.react(":three:");
            await embeds.react(":four:");
            await embeds.react(":five:");

            const songOne = (reaction, user) => reaction.emoji.name === ":one:" && user.id === message.author.id;
            const songTwo = (reaction, user) => reaction.emoji.name === ":two:" && user.id === message.author.id;
            const songThree = (reaction, user) => reaction.emoji.name === ":three:" && user.id === message.author.id;
            const songFour = (reaction, user) => reaction.emoji.name === ":four:" && user.id === message.author.id;
            const songFive = (reaction, user) => reaction.emoji.name === ":five:" && user.id === message.author.id;
